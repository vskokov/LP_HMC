...
function write_metadata(io, state, samples, skip, warmup)
    ...
    println(io, "# startup_epsilon=$(repr(Float64(startup_ε)))")
    println(io, "# startup_n_lf=$(startup_n_lf)")
    println(io, "# startup_sweeps=$(startup_sweeps)")
        ...
        open(output_tmp, "w") do stats_io
            open(diagnostics_tmp, "w") do diag_io
                ...
                diag_columns = vcat(
                    ...
                    ["round_trips_total", "exchange_rounds",
                     "walkers_with_round_trip", "walkers_visited_low",
                     "walkers_visited_high"],
                    ["round_trips_walker_$(i)" for i in eachindex(state.round_trips)],
                ...
                for sample in 1:samples
                    ...
                    diag_values = vcat(
                        ...
                        let low_coverage, high_coverage = walker_endpoint_coverage(state)
                            [sum(state.round_trips), exchange_rounds(state, swap_every),
                             count(>(0), state.round_trips), count(low_coverage),
                             count(high_coverage)]
                        end,
                        state.round_trips,
                        [stats.M, abs(stats.M), Int(ordered),
